...
//===============================================================================================
  /* 0-50 */
  char startSend[MAXBUF];
  strcpy(startSend,"start_send");

  int start_send = sendto(sk, startSend, strlen(startSend), 0,
  ...
  if (start_send != strlen(startSend)) {
    printf("Sendto sent %d bytes\n", start_send);
  }

  
  int start_read = recvfrom(sk, buf, MAXBUF, 0, NULL, NULL);
  if (start_read < 0) {
    perror("Problem in recvfrom");
    exit(1);
  }

  int countNum = atoi(buf);
  ...
  for(i=0;i<=50;i++)
  {
      ...
      size_t numChar_len = strlen(numChar);

      int sent_0_50 = sendto(sk, numChar, numChar_len, 0,
                  (struct sockaddr*) &server, sizeof(server));
      if (sent_0_50 < 0) {
          perror("Problem sending data");
          exit(1);
      }
      if (sent_0_50 != numChar_len) {
          printf("Sendto sent %d bytes\n", sent_0_50);
      }
      ...
      if(i<countNum)
      {
          for(k=0;k<MAXBUF;k++)
            buf[k] = '\0';
          int read_0_50 = recvfrom(sk, buf, MAXBUF, 0, NULL, NULL);
          if (read_0_50 < 0) {
            perror("Problem in recvfrom");
            exit(1);
          }
          if (write(STDOUT_FILENO, buf, read_0_50) < 0) {
